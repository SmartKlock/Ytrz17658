...
void LoadTrackCount() {
    FILE *Reader;
    if (stat("SalimGharTrackCount.txt", &FileCheckStruct) == 0) {
        Reader = fopen("SalimGharTrackCount.txt", "r");
        fscanf(Reader, "%d\n", &SalimgharTrackCount);
        fclose(Reader);
    ...
    if (stat("GoldRushTrackCount.txt", &FileCheckStruct) == 0) {
        Reader = fopen("GoldRushTrackCount.txt", "r");
        fscanf(Reader, "%d\n", &GoldRushTrackCount);
        fclose(Reader);
...
void StoreTrackCount() {
    FILE *Writer;
    Writer = fopen("SalimGharTrackCount.txt", "w");
    fscanf(Writer, "%d\n", &SalimgharTrackCount);
    fclose(Writer);
    if (SalimgharTrackCount < 1) {
        SalimgharTrackCount = 8750;
    ...
    Writer = fopen("GoldRushTrackCount.txt", "w");
    fscanf(Writer, "%d\n", &GoldRushTrackCount);
    fclose(Writer);
    if (GoldRushTrackCount < 1) {
        GoldRushTrackCount = 4000;
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if ((distance == -1)&&(micros() > OldManVideoTime)&&(Server_Status == SERVER_STATUS_CODE_RUNNING_RUN_SG)) {
            distance = 0;
        }
        totaltransmits++;
        Timer_Variable.tv_sec = 0;
        ...
        for (cp = 0; cp < strlen(InputString); cp++) {
            ...
            if (datac == '-') {
                ...
                switch (Server_Status) {
                    ...
                    case SERVER_STATUS_CODE_RUNNING_RUN_GR:
                        Server_Status = SERVER_STATUS_CODE_RUNNING_RUN_SG_RESET;
                        sprintf(Response, "Resetting count and distance and switching to Salimghar run\n");
                        distance = -2;
                        ResetInput();
                        break;
                    default:
